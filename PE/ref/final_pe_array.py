...
class pe_array:
    def __init__(self, num: int):
        ...
        self.ins_list = [
        "MADD","MSUB","MMUL","MMAC","PREMUL","KMUL","KMAC","NTT","INTT","P2R",
        "DCP1","DCP2","DCP3","DCP4","MHINT","UHINT","CHKZ","CHKW0","CHKH","NEQL",
        "DCMP_1","DCMP_4","DCMP_5","DCMP_10","DCMP_11",
        "CMP_1","CMP_4","CMP_5","CMP_10","CMP_11",
        "SHFL","BYPASS"]
        self.reset("DSA-44","MADD")

    def reset(self, alg: str, ins: str):
        ...
        "DCMP_1":(self.q,0), "DCMP_4":(self.q,3), "DCMP_5":(self.q,4), "DCMP_10":(self.q,9), "DCMP_11":(self.q,10),
        "CMP_1":(10079,23), "CMP_4":(315,15), "CMP_5":(630,15), "CMP_10":(5160669,23), "CMP_11":(5160670,22)}

    def check_input(self, data_in: np.ndarray):
    ...
    def madd(self,ins,in0,in1):
        match ins:
            ...
            case "DCP1":
                ...
                sub = add - cmp
                sft = sub >> 7
            case "DCP2"|"DCMP_1"|"DCMP_4"|"DCMP_5"|"DCMP_10"|"DCMP_11"|"CMP_1"|"CMP_4"|"CMP_5"|"CMP_10"|"CMP_11":
                add = in0 + 1
                cmp = 0
                sub = add - cmp
                sft = sub >> 1
    ...
    def msub(self,ins,in0,in1):
        match ins:
            ...
            case "KMUL"|"KMAC":
                ...
                cmp = np.where(sub >= 2**31, self.q, 0).astype(np.uint32) # negative when >2^31 in np.uint32
                add = sub + cmp
            case "DCP2"|"CMP_1"|"CMP_4"|"CMP_5"|"CMP_10"|"CMP_11":
                if ins=="DCP2": sub_tmp = self.alpha
                else: sub_tmp = np.uint32(1<<int(ins.split("_")[-1])) # 2^d for d = 1/4/5/10/11
                sub = in0 - sub_tmp
            ...
            case "CHKZ"|"CHKW0"|"CHKH":
                ...
                add = cmp
            case "PREMUL": # zeta[64+i], ... zeta[64+i] -> 0, zeta[64+i], 0, q-zeta[64+i]
                sub = np.array([in0[0],in0[1],in0[2],self.q],dtype=np.uint32) - np.array([in0[0],0,in0[2],in0[3]],dtype=np.uint32)
                cmp = np.where(sub >= 2**31, self.q, 0).astype(np.uint32) # redundant
                add = sub + cmp
        return add
    ...
    def exe(self, data_in: np.ndarray):
        #self.check_input(data_in)
        ...
        match self.ins: 
            ...
            case "PREMUL":
                # note that permul cannot be merged with to_mont because that requires 3 multiplication
                # for each pair (b0*mont_R, b1*mont_R, b1*mont_R*zeta)
                #data_in[0] = b0                , b1          , b2                  , b3 (pe_num=4)
                #data_in[1] = zeta[64+i]        , zeta[64+i]  , zeta[64+i]          , zeta[64+i]
                #data_out   = b0|(b1*zeta[64+i]), b0|b1       , b2|(-b3*zeta[64+i]) , b2|b3
                tmp = self.msub("PREMUL",data_in[1],None)   # 0, zeta[64+i]   , 0, -zeta[64+i]
                tmp2 = self.mmul("MMUL",data_in[0],tmp)     # 0, b1*zeta[64+i], 0, -b3*zeta[64+i]
                for i in range(self.num//2):
                    self.data_out[0][2*i  ] = (data_in[0][2*i]<<12)|(tmp2      [2*i+1])
                    self.data_out[0][2*i+1] = (data_in[0][2*i]<<12)|(data_in[0][2*i+1])
            ...
            case "P2R":
                ...
                tmp1 = self.msub("P2R",data_in[0],tmp0)
            ...
            case "DCP2"|"CMP_1"|"CMP_4"|"CMP_5"|"CMP_10"|"CMP_11":
                tmp0 = self.mmul(self.ins,data_in[0],None)        # (input*1025>>21)
                tmp1 = self.madd(self.ins,tmp0,None)              # (input+1)>>1
                self.data_out[0] = self.msub(self.ins,tmp1,None)  # input%16
            ...
            # Third stage of decompose, but only return the sign of a0
            # For unsigned data format: a0' = (a-a1*2*GAMMA2) mod q
            # a0>0 iff  a0'<=(Q-1)/2 and a0'!=0
            case "DCP4":
                tmp0 = self.mmul("DCP3",data_in[1],None)
                tmp1 = self.msub("MSUB",data_in[0],tmp0)
                tmp2 = self.madd("DCP4",tmp1,None)
                self.data_out[0] = tmp2
            ...
            case "CHKZ"|"CHKW0"|"CHKH":
                tmp0 = self.madd(self.ins,data_in[0],None)  # >= bound
                tmp1 = self.msub(self.ins,data_in[0],None)  # <= q-bound
                self.data_out[0] = tmp0 & tmp1

            # Decompression: round(input*q/2^d) = (input * q + 2^(d-1)) >> d
            # = (((input * q) >> (d-1) + 1) >> 1 
            case "DCMP_1"|"DCMP_4"|"DCMP_5"|"DCMP_10"|"DCMP_11":
                tmp0 = self.mmul(self.ins,data_in[0],None)        # (input*constant A>>constant B)
                self.data_out[0] = self.madd(self.ins,tmp0,None)  # (input+1)>>1
            
            case "NEQL":
                tmp0 = self.msub("NEQL",data_in[0],data_in[1]) # in0 - in1 with 24-bit datawidth
                self.data_out[0] = self.madd("NEQL",tmp0,None)
            
            case "SHFL":
                self.data_out[0] = self.msub("SHFL",None,data_in[0])
            
            case "BYPASS":
                self.data_out[0] = data_in[0]
